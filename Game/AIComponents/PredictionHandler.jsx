...
const PredictionHandler = ({
  ...
  // Add effect to auto-analyze when the prop is true
  useEffect(() => {
    if (autoAnalyze && imageData && !isLoading) {
  ...
  const handleSubmitAndAnalyze = async () => {
    ...
    // Only call onAnalyze if not auto-analyzing
    if (!autoAnalyze) {
      onAnalyze();
    }
  ...
  // If in auto-analyze mode, just show loading state
  if (autoAnalyze) {
    return (
  ...
  // Regular button view for manual submission
  return (
    <div className="space-y-4">
      <button
